...
static const unsigned char avs_icon [] = {
0x10, 0x10,
0x00, 0xE0, 0xF8, 0x3C, 0x9C, 0x1E, 0x76, 0xF6, 0xC6, 0x0E, 0x7E, 0xFC, 0x7C, 0x98, 0xC0, 0x00,
0x00, 0x0B, 0x19, 0x3E, 0x3F, 0x7E, 0x70, 0x63, 0x6F, 0x6E, 0x78, 0x3B, 0x3C, 0x1F, 0x07, 0x00
};

static const unsigned char wifi_icon [] = {
0x10, 0x10,
0x00, 0x60, 0x30, 0xB0, 0x98, 0xD8, 0xD8, 0xC8, 0xC8, 0xD8, 0xD8, 0x98, 0xB0, 0x30, 0x60, 0x00,
0x00, 0x00, 0x00, 0x01, 0x01, 0x04, 0x06, 0x16, 0x16, 0x06, 0x04, 0x01, 0x01, 0x00, 0x00, 0x00
};
...
int oled_page_init(void) {
    ...
    OLED_createTextField(&humi_meas_text_id, 99U, 10U, humi_meas_txt, 1U, false);
...
int oled_avs_icon(bool enable) {
    ...
    if (enable && !enabled && !(ret = OLED_createImage(&avs_icon_id, 70, 0, avs_icon))) {
        enabled = true;
...
int oled_wifi_icon(bool enable) {
    ...
    if (enable && !enabled && !(ret = OLED_createImage(&wifi_icon_id, 40, 0, wifi_icon))) {
        enabled = true;
    } else if (!enable && enabled){
