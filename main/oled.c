...
static void sendCommandStream(const uint8_t *stream, uint16_t streamLength)
{
    assert(streamLength);
...
static void setAddress(uint8_t page, uint8_t column)
{
    ...
    i2c_master_write_slave_reg(&oled_device, OLED_CONTROL_BYTE_ | _OLED_COMMAND | _OLED_MULTIPLE_BYTES, addressArray, 3);
}

static void setPixel(uint8_t x, uint8_t y)
{
    *(oled_ctx.buffer + x + OLED_X_SIZE*( (uint8_t) (y / 8) )) |= (0x01 << y % 8);
}
...
static void drawLine(uint8_t x0, uint8_t y0, uint8_t x1, uint8_t y1)
{
    ...
    if(1)
    {
        ...
        {
            *(oled_ctx.buffer + (( uint8_t ) y / 8)*OLED_X_SIZE + ( uint8_t ) x ) |= 1 << ( (uint8_t) y % 8);
            yTemp += tan;
            if(yTemp >= 1)
            {
                if(yDir == 1)
...
static void drawRect(uint8_t x0, uint8_t y0, uint8_t x1, uint8_t y1, enum OLED_Color color){
    ...
    while(c <= x1)
    {
        if(color == WHITE)
            if(y1 / 8 == y0 / 8)
                *(oled_ctx.buffer + v*OLED_X_SIZE + c++) |= (0xFF << rem0) & (0xFF >> (8 - rem1));
            else
    ...
    if((y1 % 8) && (y1 / 8 != y0 / 8))
    {
        ...
        {
            if(color == WHITE)
                *(oled_ctx.buffer + v*OLED_X_SIZE + c++) |= 0xFF >> ( 8 - rem1 );
            else
                *(oled_ctx.buffer + v*OLED_X_SIZE + c++) &= ~(0xFF >> ( 8 - rem1 ));
        }
    }
...
static void drawImage(uint8_t x0, uint8_t y0,const uint8_t image[])
{
    ...
    else
    {
        ...
        while( (v <= y1 / 8) && (v < 8) )
        {
            ...
            while(c <= x1)
            {
                *(oled_ctx.buffer + v*OLED_X_SIZE + c) |= image[i - width] >> (8 - rem0);
                if( (v != y1 / 8) && (v < 8) )
                    *(oled_ctx.buffer + v*OLED_X_SIZE + c) |= image[i] << rem0;
                i++;
                c++;
